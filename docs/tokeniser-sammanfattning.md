...
| Q16 | **Mätvärden sparas bara när de ändras.** Rådata sparas i 90–121 dagar, eftersom en hel månadsfil rensas åt gången (ändrat 2026-09-16). Dagssummeringar sparas tills du raderar dem. Det som **inte** sparas beskrivs i avsnitt 7. **Dagssummor av tokens** är en undre gräns, eftersom subagenternas anrop aldrig syns i statusraden (orsaken rättad 2026-09-17, se avsnitt 12). De märks alltid "uppskattning" (beslutat 2026-09-14). |
...
5. Implementation enligt avsnitt 8. Varje större del kräver fortfarande ditt godkännande.
   ...
   - **Godkänd 2026-09-15:** hälsoraden i skissen (punkt 7). Den visar senaste data, fält i datan, insamlaren, mapparna, statusraden i Claude Code, Node-filen och det som är utom räckhåll. Varje kontroll har läget I ordning, Varning, Kan inte kontrolleras eller Kontrolleras inte. Ett fält som saknas av en dokumenterad orsak räknas som i ordning. Svar på skissens tre frågor:
     - hälsoraden ligger alltid överst i vyn, före förslaget, och är utfälld bara när något är fel
     - "Kontrolleras inte" och "Utom räckhåll" står i listan med de andra kontrollerna
     - gränsringarna märks inte som osäkra när insamlaren har ändrats; varningen i hälsoraden, ikonen i statusraden och raden i snabbkortet räcker
   - **Beslutat 2026-09-15 om hälsokontrollen:**
   ...
   - **Undersökt 2026-09-17:** statusraden jämförd med Claude Codes sessionsloggar, efter en rapport från en annan session. Från loggarna lästes bara siffror, aldrig innehåll, och Tokeniser läser dem fortfarande inte (Q6).
     - **Anropen stämmer exakt:** i de 7 sessioner som insamlaren såg från början gav statusraden samma antal anrop och tokens som huvudloggen, räknat en gång per `message.id`. Rättelsen av dubbelräkningen från 2026-09-16 stämmer alltså, och sammanslagningen inom 300 ms missade inget anrop.
     - **Subagenter syns inte:** deras anrop ligger i egna loggar och kommer aldrig till statusraden. I en session gjorde en subagent 43 anrop med 2,3 M tokens, ungefär 13 % av sessionens tokens. Det är den verkliga orsaken till att summorna är en undre gräns.
